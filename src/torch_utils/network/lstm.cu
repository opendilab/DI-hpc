...
void LstmForward(
    ...
    const float* inputptr = x0.data_ptr<float>();
    const float* h0ptr = h0.data_ptr<float>();
    const float* c0ptr = c0.data_ptr<float>();
    const float* wxptr = wx.data_ptr<float>();
    const float* whptr = wh.data_ptr<float>();
    const float* biasptr = bias.data_ptr<float>();
    const float* ln_gammaptr = ln_gamma.data_ptr<float>();
    const float* ln_betaptr = ln_beta.data_ptr<float>();
    float* xbufptr = xbuf.data_ptr<float>();
    float* hbufptr = hbuf.data_ptr<float>();
    float* hnptr = hn.data_ptr<float>();
    float* cnptr = cn.data_ptr<float>();
    float* ifogptr = ifog.data_ptr<float>();
    float* outputptr = ym.data_ptr<float>();
    float* ln_in_ptr = ln_in.data_ptr<float>();
    float* ln_mean_ptr = ln_mean.data_ptr<float>();
    float* ln_rstd_ptr = ln_rstd.data_ptr<float>();
    unsigned int* maskptr = reinterpret_cast<unsigned int*>(dropout_mask.data_ptr<int32_t>());
    float onedata = 1;
    float zerodata = 0;
...
void LstmBackward(
    ...
    const float* x0ptr = x0.data_ptr<float>();
    const float* ifogptr = ifogbuf.data_ptr<float>();
    const float* ymptr = ym.data_ptr<float>();
    const float* h0ptr = h0.data_ptr<float>();
    const float* c0ptr = c0.data_ptr<float>();
    const float* hnptr = hn.data_ptr<float>();
    const float* cnptr = cn.data_ptr<float>();
    const float* wxptr = wx.data_ptr<float>();
    const float* whptr = wh.data_ptr<float>();
    const float* ln_in_ptr = ln_in.data_ptr<float>();
    const float* ln_mean_ptr = ln_mean.data_ptr<float>();
    const float* ln_rstd_ptr = ln_rstd.data_ptr<float>();
    const float* ln_gammaptr = ln_gamma.data_ptr<float>();
    const unsigned int* maskptr = reinterpret_cast<unsigned int*>(dropout_mask.data_ptr<int32_t>());
    float* dgatebufptr = dgatebuf.data_ptr<float>();
    float* xbufptr = xbuf.data_ptr<float>();
    float* hbufptr = hbuf.data_ptr<float>();
    float* dyptr = dy.data_ptr<float>();
    float* dxptr = dx.data_ptr<float>();
    float* dhptr = dh.data_ptr<float>();
    float* dcptr = dc.data_ptr<float>();
    float* dwxptr = dwx.data_ptr<float>();
    float* dwhptr = dwh.data_ptr<float>();
    float* dbiasptr = dbias.data_ptr<float>();
    float* ln_dgammaptr = d_ln_gamma.data_ptr<float>();
    float* ln_dbetaptr = d_ln_beta.data_ptr<float>();
    float onedata = 1;
    float zerodata = 0;
